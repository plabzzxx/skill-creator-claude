...
>
> **If you are NOT in Claude Code:** skip `run_loop.py` entirely. Instead, propose description improvements yourself based on the eval queries the user reviewed, then ask the user to test them manually. You can still use `improve_description.py` standalone if `ANTHROPIC_API_KEY` is available.
